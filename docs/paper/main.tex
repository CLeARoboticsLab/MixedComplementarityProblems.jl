...
\begin{subequations}
  ...
  \begin{align}
    ...
    &0 \le \underbrace{h(x; \theta)}_{H(x,\lambda, \mu;\theta)} \perp \mu \ge 0.
  \end{align}
\end{subequations}
...
Concretely, concatenate the agents' primal variables as $x=(x_1,\dots,x_N)$ and their
private multipliers as $\lambda_\mathrm{priv}=(\lambda_1,\dots,\lambda_N)$,
$\mu_\mathrm{priv}=(\mu_1,\dots,\mu_N)$, and append a single pair of shared multipliers
$\lambda_\mathrm{sh},\mu_\mathrm{sh}$ --- common to \emph{every} agent's stationarity
condition, rather than duplicated per agent, as is standard when seeking a variational
(normalized) equilibrium \cite{facchinei2003finite} --- so that agent $i$'s Lagrangian
is $L_i \coloneqq f_i - \lambda_i^\top g_i - \mu_i^\top h_i - \lambda_\mathrm{sh}^\top
g_\mathrm{sh} - \mu_\mathrm{sh}^\top h_\mathrm{sh}$. The concatenated system is again an
instance of \cref{eq:primal-dual-mcp}, with
\begin{equation}
  \begin{aligned}
    G &\coloneqq \left(\nabla_{x_1} L_1, \dots, \nabla_{x_N} L_N, g_1,\dots,g_N,
    g_\mathrm{sh}\right), \\
    H &\coloneqq \left(h_1,\dots,h_N,h_\mathrm{sh}\right),
  \end{aligned}
  \label{eq:game-mcp}
\end{equation}
with $G$ and $H$ each a function of the full concatenated primal and dual variables
and $\theta$. Every agent's stationarity block $\nabla_{x_i}L_i$ depends on the shared
multipliers
$\lambda_\mathrm{sh},\mu_\mathrm{sh}$ in addition to its own $\lambda_i,\mu_i$, which is
exactly what couples the agents' otherwise-separate KKT systems into one game.

\begin{runningexample}{lane-change trajectory game}
